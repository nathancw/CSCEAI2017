...
//					System.out.print(Double.toString(nn.layers.get(i).hinge[j]) + ", ");
		...
		static double[] evolveWeights()
		{
			...
			for(int x = 0; x < numTournaments; x++){
				...
				if(cNum1 != cNum2 && chromoOne[0]!=0.0 && chromoTwo[0] !=0.0){
					...
					if(winner == 1){
						...
						if(killLoser <= probToSurvive){ //% to killLoser and live
			...
			for(int i = 0; i < 100; i++){
				...
				if(population.row(i)[0] == 0.0){
					...
					//System.out.println(" New child " + i + " : " + Arrays.toString(population.row(i)));
				}
			}
	...
	public static double[] getWeightsOnly(double[] c){
		...
		public static void main(String[] args) throws Exception
		{
			...
			//System.out.println(Controller.doBattleNoGui(new ReflexAgent(), new NeuralAgent(getWeightsOnly(w)))); //Looking for -1
